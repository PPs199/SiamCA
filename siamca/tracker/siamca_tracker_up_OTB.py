...
class SiamCATracker(SiameseTracker):
    ...
    def track(self, img):
       ...
       # if(self.best_score<0.80 and self.lostnum>3):
         #    w_z = self.size[0]*1.25 + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size*1)
           #  h_z = self.size[1]*1.25 + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size*1)
           #  self.lostnum=0
      #  else:
       #      w_z = self.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
       #      h_z = self.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
    ...
    #丢失重搜索
       ...
       # else:
        ...
        if self.frame_id > 100   and self.best_score > 0.75 and self.lostnum==0 :#帧数为200就更新
            #先获取 更新模板的裁剪图片 和 roi 

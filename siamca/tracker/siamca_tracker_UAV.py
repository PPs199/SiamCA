...
import os
class SiamCATrackerUAV(SiameseTracker):
    def __init__(self, model):
        super(SiamCATrackerUAV, self).__init__()
    ...
    def init(self, img, bbox,vedioname):
        ...
        self.c = (cfg.TRACK.EXEMPLAR_SIZE - 1) / 2 
    ...
    def track(self, img):
        ...
        if(self.best_score<0.96 and self.lostnum>2):
             w_z = self.size[0]*1.2 + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size*1)
             h_z = self.size[1]*1.2 + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size*1)
             self.lostnum=0
        else:
             w_z = self.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
             h_z = self.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        ...
        ##
        ##judge lost statu
        if self.best_score<0.96 and self.last_score<0.96:
        ...
        if self.frame_id > 100   and self.best_score > 0.99  and self.lostnum==0:

...
from siamca.tracker import utils
from matplotlib import pyplot as plt 
class SiamCATracker(SiameseTracker):
    def __init__(self, model):
        super(SiamCATracker, self).__init__()
    ...
    def init(self, img, bbox,vedioname):
        """
        args:
    ...
    def track(self, img):
        ...
        """
        #丢失重搜索【
        if(self.best_score<0.85 and self.lostnum>3):
        ...
        x_crop = self.get_subwindow(img, self.center_pos,
                                    ...
                                    round(s_x), self.channel_average,self.vedioname,self.frame_num)
        self.frame_id += 1
        self.frame_num +=1
        outputs = self.model.track(x_crop)
##用于热力图
       # utils.heat_map(outputs['cls'],self.vedioname,self.frame_num)
##
    ...
    #丢失重搜索
        ...
        if self.frame_id > 100   and self.best_score > 0.9 and self.lostnum==0 :#帧数为200就更新
            #先获取 更新模板的裁剪图片 和 roi 
             w_z = self.size[0]*0.75 + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
             h_z = self.size[1]*0.75 + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
             s_z = round(np.sqrt(w_z * h_z))
